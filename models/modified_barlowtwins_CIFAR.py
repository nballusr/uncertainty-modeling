...
from loss import ModifiedBarlowTwinsLoss


class ModifiedBarlowTwinsModel(BenchmarkModule):
    def __init__(self, dataloader_kNN, gpus, knn_k, knn_t, cosine_schedule_length, num_mlp_layers, initial_lr,
        ...
        # note that modified barlowtwins has the same architecture
        self.resnet_simsiam = \
            lightly.models.SimSiam(self.backbone, num_ftrs=512, num_mlp_layers=num_mlp_layers)
        device = 'cuda' if gpus else 'cpu'
        self.criterion = ModifiedBarlowTwinsLoss(device=device)
    ...
    def training_step(self, batch, batch_idx):
        ...
        z_a, _ = x0
        z_b, _ = x1
        loss = self.criterion(z_a, z_b, labels)
